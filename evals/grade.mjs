...
function main() {
  ...
  // Fused dispatch: the workspace grader and logo check live alongside the gate CLI
  if (args.includes("--check-logo")) {
    const target = args[args.indexOf("--check-logo") + 1];
    if (!target) { console.error("usage: node evals/grade.mjs --check-logo <file.svg>"); process.exit(2); }
    const abs = path.isAbsolute(target) ? target : path.resolve(process.cwd(), target);
    const results = checkLogoQuality(fs.readFileSync(abs, "utf8"));
  ...
  if (args.includes("--workspace") || args.includes("--iteration")) {
    const wsGrader = path.join(path.dirname(fileURLToPath(import.meta.url)), "grade-workspace.mjs");
    const r = spawnSync(process.execPath, [wsGrader, ...args], { stdio: "inherit" });
